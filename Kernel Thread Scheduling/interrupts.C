...
/* This macro is defined, user scheduler would take Round-Robin method for ready threads.
   ...
   this macro decides which code would be compiled in interrupt.C
   in Round-Robin mode: interrupt.C, handler first sends EOI to PIC, and then handle interrupt. 
   in FIFO mode: interrupt.C, handler first handles interrupt, and then send EOI to PIC. 
*/
/* (none) */
...
void InterruptHandler::dispatch_interrupt(REGS * _r) {
  ...
  if (!handler) {
	...
	//Before handle interrupt, send EOI PIC first. Becasue now there is other interrupt in the handler,
    //there is context switch which has to process when schduler changes thread.	
	if (generated_by_slave_PIC(int_no)) {
		Machine::outportb(0xA0, 0x20);
	}
	/* Send an EOI message to the master interrupt controller. */
